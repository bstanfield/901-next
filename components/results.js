...
export default function Results({
  ...
  const cocktailsToDisplay = cocktails
    .slice(0, displayMaximum || 901)
    .sort((a, b) => {
  ...
  if (pantry && keywords.length >= 1) {
    ...
    const cocktailsSortedByPrimaryKeyword = cocktailsToDisplay.sort((a, b) => {
      if (
        a.ingredients.includes(primaryKeyword.value) &&
        !b.ingredients.includes(primaryKeyword.value)
      ) {
        return -1;
      }
      if (
    ...
    return cocktailsSortedByPrimaryKeyword.map((cocktail) => (
      <Cocktail
        mapping={mapping}
        key={cocktail.id}
        keywords={keywords}
    ...
    return cocktailsToDisplay.map((cocktail) => (
      <Cocktail
        mapping={mapping}
        key={cocktail.id}
        keywords={keywords}
