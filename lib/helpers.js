...
export const getPopularIngredients = (cocktails, keywords) => {
  ...
  const preciseEntries = entries.slice(0, 4).map(entry => {
    return { ingredient: entry.ingredient, count: improvedGetRelevantCocktails(cocktails, [{ value: entry.ingredient, label: entry.ingredient, type: "positive", bgColor: "rgb(221, 237, 255)" }]).length }
  })
  console.log('precise entries: ', preciseEntries)
