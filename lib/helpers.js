...
export const getRelevantCocktails = (cocktails, searchFilters, lists, sortingRule) => {
  console.log('getting relevant cocktails...')
  const searchFiltersMatches = getCocktailsBySearch(cocktails, searchFilters)
  const listAndSearchFiltersMatches = getCocktailsByLists(searchFiltersMatches, lists)
...
const getCocktailsByLists = (cocktails, lists) => {
  // ['sweet', 'simple']
  ...
  const relevantCocktails = cocktails.filter(cocktail => {
    ...
    for (const list in lists) {
      console.log('checking list: ', lists[list])
      if (!cocktail.lists.includes(lists[list])) {
        fitsAllCriteria = false
        break;
      }
      fitsAllCriteria = true
    }
    if (fitsAllCriteria === true) {
      return true
    }
    return false
  })
  console.log('relevant cocktails: ', relevantCocktails)
  return relevantCocktails
}