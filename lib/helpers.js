...
export const getPopularIngredients = (allCocktails, cocktails, keywords, pantry) => {
  ...
  const preciseEntries = entries.slice(0, 4).map(entry => {
    return { ingredient: entry.ingredient, count: improvedGetRelevantCocktails(pantry ? allCocktails : cocktails, pantry ? [...keywords, { value: entry.ingredient, label: entry.ingredient, type: "positive", bgColor: "rgb(221, 237, 255)" }] : [{ value: entry.ingredient, label: entry.ingredient, type: "positive", bgColor: "rgb(221, 237, 255)" }], pantry).length }
  })
