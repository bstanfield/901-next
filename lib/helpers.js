...
export const getRelevantCocktails = (cocktails, searchFilters, negativeSearchFilters) => {
  ...
  const relevantCocktails = cocktails.filter(cocktail => {
    ...
    // console.log(`checking if cocktail ${cocktail.name} has negative filters: ${JSON.stringify(negativeSearchFilters)}`);
    if (negativeSearchFilters.length > 0) {
      negativeSearchFilters.map(negativeFilter => {
        if (cocktail.name.includes(negativeFilter) || cocktail.lists.includes(negativeFilter)) {
          console.log(cocktail.name, ' has a negative filter: ', negativeFilter)
          count = 0;
        } else {
          if (cocktail.ingredients.map(ingredient => { if (ingredient.includes(negativeFilter)) { return true } else { return false } }).includes(true)) {
           console.log(cocktail.name, ' has a negative filter: ', negativeFilter)
           count = 0;
          }
      }
...
export const createSentence = (filters, negativeFilters) => {
  let str = ''
  for (const filter in filters) {
    if (filter == 0) {
      str = str + '<span style="opacity: .6">with </span>' + filters[filter]
    } else {
      str = str + '<span style="opacity: .6"> & </span>' + filters[filter]
    }
  }
  for (const negativeFilter in negativeFilters) {
    if (negativeFilter == 0) {
      str = str + '<span style="opacity: .6"> and no </span>' + negativeFilters[negativeFilter]
    } else {
      str = str + '<span style="opacity: .6"> & </span>' + negativeFilters[negativeFilter]
    }
  }
  return str
}
